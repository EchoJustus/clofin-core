...
| **Status** | `CLOSED` — merged to `main` in PR #6 (`2ba977e`) 2026-08-04; all three objections ruled in the Worker's favour, see Changelog. C-05 is unqualified on `main`. **Fully clean only as of PR #7 (`cba31c5`)**, which landed the post-merge contract fix this ingestion missed — see the Changelog tail and lesson L-9. **Audited in [FEEDBACK-M2](../audits/FEEDBACK-M2-settlement-and-audit-coverage.md): APPROVED-WITH-CONDITIONS** — the condition is F-011 (services accept a pool; the transaction precondition must fail closed), actioned in the TASK-004 remediation batch and recorded as lesson L-13 |
