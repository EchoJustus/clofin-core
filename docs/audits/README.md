...
items performed** — the first complete release audit; the resource-interruption
fallback was not needed. Sessions A–C ran on GPT-5.6 Sol (2026-09-05/06). The
operator's allocation ran out after session C had completed items 3–4;
**session D** — item 8 over 230 citations (127 matches, 99 corrected with the
finding retained, 4 historical diagnostics excluded, none discarded) and the
report — ran the same day on purchased allocation, from a prompt issued in chat
and recorded verbatim in
[`REL-ref-2-SESSION-D-PROMPT.md`](REL-ref-2-SESSION-D-PROMPT.md); D records its
model as "GitHub Copilot; underlying model not exposed", carried as stated (the
known-and-withheld form the 014-REQ precedent admits). **Result: 2 blocking /
20 should-fix / 1 consider**, plus one unnumbered residual (C-R10a) confirmed
by D. Severities are the auditor's throughout — the first release audit whose
severities Master Control did not have to assign. **Both blockers were
independently confirmed in source by Master Control before this block was
written**: 2C-006 — `start!` in `tools/clofin/tools/capture/stack.clj` accepts
a `200` on `/readyz` *before* asking whether the spawned child is alive, and
`assert-schema-matches!` compares schema versions, which is not identity;
2C-002 — both receipt-collision branches in `clofin.recon.service` replay the
winner's receipt without the digest comparison the serial path makes, so a
document nobody processed is acknowledged as an exact replay. **The RC
`c97a4f2` is not taggable.** `ref-2` lands on the remediation descendant once
both blockers are re-verified by the Worker's regression tests and by Master
Control's reproduction. **Dispositions, by class.** *Actioned in the subject
via [TASK-015](../briefs/015-TASK-ref-2-release-remediation.md)* (the
data-plane path, amendment A2): 2C-002, 2C-006, 2B-003, 2B-004, 2B-005, 2B-007,
2B-009, 2B-010, 2C-001, 2C-003, 2C-004, 2C-005, 2C-007, 2C-009, 2C-010, 2C-011,
2C-012 (a *consider*, actioned because its fix shares 2C-011's file), C-R10a,
with 010-REQ N-3 riding beside C-R10a. *Actioned on `meta` directly* (doc-only,
meta-owned documents): 2B-006 (three ROADMAP carried-forward bullets), 2B-011
(repaired at commissioning; recorded), 2C-008 (the pickup paragraph no longer
names a task), 2B-001 and 2B-002 (AGENT_HANDOFF §4: the migration pre-flight
and the dependency matrix become template sections, and TASK-015 is the first
brief written under them). *Deferred with a target*: 2B-008 — the `ref-1`
release body is a published artifact and is not rewritten, by the same rule
that left `ref-1` lightweight; from `ref-2` onward every annotation carries the
canonical disclaimer verbatim and a mechanical check enforces it (TASK-015),
and the `ref-1` omission is recorded in `docs/releases/README.md`. *Disputed*:
none; all four pre-declared inputs reproduced. **The tier-correlated
miss-pattern**, which is what the gate exists to produce: *a complete-looking
check over one dimension of a larger set* — required fields for one writer of
four (2C-005), the initial lookup but not the collision recovery (2C-002),
top-level enums but not their property uses, with the property check itself
vacuous (2C-003), status rows but not prose restatements (2B-011, 2C-008), one
posting producer but not every producer (2C-009). Five instances, all green at
the continuous tier. Adopted as **L-17**; the audit's L-18…L-21 are adopted
below; its L-22 (citation provenance) amends the charter's rule 2 rather than
the product lessons. The deferral notes of 2026-08-14 and 2026-08-15 are
**discharged**: increments 5v.1–8.4 have received their independent audit.
`meta` writes resume with this block. *REQ keying:* TASK-015 is a brief, so its
report is the task-keyed `015-REQ`; the `REL-<tag>-REQ` form remains for a
remediation that has no brief.
...
| [`FEEDBACK-REL-ref-2`](FEEDBACK-REL-ref-2.md) — **ingested (COMPLETE, 8 of 8)** | Release audit of the `ref-2` candidate, `main` `c97a4f2` — full whole-repo, delta-ordered; commission [`REL-ref-2-COMMISSION.md`](REL-ref-2-COMMISSION.md), session D prompt [`REL-ref-2-SESSION-D-PROMPT.md`](REL-ref-2-SESSION-D-PROMPT.md) | commissioned 2026-09-05 · delivered 2026-09-06 · ingested 2026-09-06 | **2 B / 20 S / 1 C** (+1 unnumbered residual); severities the auditor's | **All eight charter items performed** — the first complete release audit; four sessions (A: items 1–2 · B: items 5–7, mandatory-first from `ref-1` · C: items 3–4 delta-first · D: item 8 and the report, after C was cut off by the operator's allocation). **Both blocking findings independently confirmed in source by Master Control** — 2C-002 (receipt-collision branches replay without the digest comparison) and 2C-006 (capture accepts a foreign process's readiness before checking its own child). **RC not taggable**; remediation dispatched as [TASK-015](../briefs/015-TASK-ref-2-release-remediation.md) on the data-plane path. Dispositions: 18 actioned in the subject via TASK-015 (+ 010-REQ N-3 riding), 5 actioned on `meta` (2B-001, 2B-002, 2B-006, 2B-011, 2C-008), 1 deferred with a target (2B-008), 0 disputed — detail in the decision block of 2026-09-06. Lessons **L-17…L-21** adopted; L-22 amends the charter. Workpapers A–C and D's citation evidence live in the bridge at `clofin-core/audit/workpapers/ref-2/` (workpapers are not repository artifacts). `ref-2` lands on the remediation descendant. | GPT-5.6 Sol (sessions A–C) · session D "GitHub Copilot; underlying model not exposed" · CodeSpace path · 2026-09-05/06 · effort not recorded |
...
| L-17 | **A guard complete along one dimension of its set and absent along another.** The `ref-2` audit's tier-correlated miss-pattern, five instances green at the continuous tier: required provenance fields checked for one capture writer of four (2C-005); a statement's digest checked on the initial lookup but not in the collision recovery (2C-002, blocking); OpenAPI enums compared at top level but not in their property uses — and the property check itself vacuous, because the YAML parser returned Java maps that `map?` rejects (2C-003); status rows compared by the doc guard but not prose restatements (2B-011, 2C-008); one journal-posting producer audited but not every producer (2C-009). | When writing or reviewing a guard, enumerate its **dimensions** — member set, entry path, output sink, boundary case — not only its members. **Assert non-vacuity** after every parse or discovery step: a discovered population of zero fails. Put **one mutation that must fail** beside every positive assertion, and show it failing. AGENT_HANDOFF §4's template asks for the negative control by name; TASK-015 applies this to every guard it touches. |
| L-18 | **Concurrency arbitration re-decided nothing.** A receipt-insert collision's loser replayed the winner's receipt without the digest comparison the serial path makes, so a document nobody processed was acknowledged as an exact replay of a different one (2C-002, blocking; the serial path correctly answers `409`). | After any arbitration — a unique-key collision, a lock wait, a lost race — the loser re-enters **the same decision the serial path makes**: identity, digest, outcome. Never a shortcut that assumes equivalence because the key matched. A two-connection test covers identical and different content for every arbitration branch. Extends L-12. |
| L-19 | **Captured evidence was bound to a port and a schema version, neither of which is an identity.** The capture harness accepted a `200` on `/readyz` before checking whether the child it spawned was alive, and its identity check compared schema versions; a same-schema process already listening would have been stamped as the commit under capture (2C-006, blocking). | A capture binds to **the process it started**: a per-run identity the child echoes, liveness checked before acceptance and before every write, an occupied port refused before spawning. Port and schema version remain checks; they are never identity. The tests include an unrelated same-schema responder. |
| L-20 | **An acceptance script inherited state that invalidated its own example.** UAT-007 kept UAT-006's from-zero approval band, so its de-minimis case could not post as written (2C-004); UAT-006's evidence step named an actor the role table refuses (2B-010, pre-declared). Both scripts read as complete. | Acceptance scripts are **executable contracts**: a sequel replays its prerequisite's roles, variables and seeded configuration and asserts the boundary it promises; a script that names an actor names one the permission table grants; every affected step is run against a fresh stack before the REQ is filed. Extends L-10. |
| L-21 | **Audit coverage followed the first producer, not every producer.** Settlement posted journal entries through the ledger repository rather than the service that emits `journal-entry.posted`, so those entries had no event and no evidence pack (2C-009); adjustment evidence did not traverse the approval relationship a payment's does (2C-012). | The audited set is **every committing producer and every subject relationship**, discovered from the code: a test enumerates the callers of each write primitive and asserts each emits, and evidence extraction is tested per subject type with its full relation set. Extends L-7 and L-13. |
